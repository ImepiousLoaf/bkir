...
void ATRXPlayerControllerBase::SetupInputComponent()
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(InputComponent))
	{
		...
		EnhancedInputComponent->BindAction(Ability1, ETriggerEvent::Started, this, &ATRXPlayerControllerBase::Ability1Input);
		EnhancedInputComponent->BindAction(Ability2, ETriggerEvent::Started, this, &ATRXPlayerControllerBase::Ability2Input);
		EnhancedInputComponent->BindAction(Ability3, ETriggerEvent::Started, this, &ATRXPlayerControllerBase::Ability3Input);
		EnhancedInputComponent->BindAction(Ability4, ETriggerEvent::Started, this, &ATRXPlayerControllerBase::Ability4Input);
		EnhancedInputComponent->BindAction(Ability1, ETriggerEvent::Completed, this, &ATRXPlayerControllerBase::Ability1Input);
		EnhancedInputComponent->BindAction(Ability2, ETriggerEvent::Completed, this, &ATRXPlayerControllerBase::Ability2Input);
		EnhancedInputComponent->BindAction(Ability3, ETriggerEvent::Completed, this, &ATRXPlayerControllerBase::Ability3Input);
		EnhancedInputComponent->BindAction(Ability4, ETriggerEvent::Completed, this, &ATRXPlayerControllerBase::Ability4Input);
	}
}
...
void ATRXPlayerControllerBase::PlayerTick(float DeltaTime)
{
	...
	if(IsLocalController() && GetPawn())
	{
		...
		const FRotator& NewRotation = FQuat::Slerp(ActorRotation.Quaternion(), TargetRotator.Quaternion(), RotationSpeed * DeltaTime).Rotator();
		...
		if(DebugTrace)
		{
			...
			DrawDebugLine(GetWorld(), GetPawn()->GetActorLocation(), GetPawn()->GetActorLocation() + ActorRotation.Vector()*300, FColor::Red, false, 0);
			auto HitResult = GetCursorLoc();
			auto tr = FRotationMatrix::MakeFromX(HitResult.Location-GetPawn()->GetActorLocation()).Rotator().Yaw;
			DrawDebugLine(GetWorld(), GetPawn()->GetActorLocation(), GetPawn()->GetActorLocation() + FRotator(0,tr,0).Vector()*300, FColor::Green, false, 0);	
		}
	}
	
}

void ATRXPlayerControllerBase::GetAudioListenerPosition(FVector& OutLocation, FVector& OutFrontDir, FVector& OutRightDir) const
{
